      ...
      <form onSubmit={submit} className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1fr_25rem]">
        <section>
          ...
          <div className="mt-8 grid gap-6">
            ...
            <div className="rounded-[2rem] bg-white/80 p-6 stripe-shadow"><h2 className="text-2xl font-semibold">Delivery options</h2><div className="mt-4 grid gap-3"><label className="flex cursor-pointer items-center justify-between rounded-2xl border border-[#0b5f59] bg-[#e8f0ef] p-4"><span><b>Standard insured shipping</b><span className="block text-sm text-[#334155]">4–7 business days</span></span><span>Free</span></label><label className="flex cursor-pointer items-center justify-between rounded-2xl border border-slate-200 p-4"><span><b>Express shipping</b><span className="block text-sm text-[#334155]">2–3 business days</span></span><span>$18</span></label></div></div>
        ...
        <aside className="h-fit rounded-[2rem] bg-[#11263d] p-6 text-white stripe-shadow"><h2 className="text-2xl font-semibold">Order summary</h2><div className="mt-5 space-y-4">{details.lines.map((line) => <div key={`${line.slug}-${line.lens}-${line.frameSize}`} className="rounded-2xl bg-white/10 p-4"><div className="flex justify-between gap-3"><b>{line.product?.name}</b><span>x{line.qty}</span></div><p className="mt-1 text-sm text-[#d7e3e1]">{line.lens} • {line.frameSize} • {line.color}</p></div>)}</div><div className="mt-6 space-y-3 text-sm text-[#e8f0ef]"><div className="flex justify-between"><span>Subtotal</span><b className="text-white">{formatMoney(details.subtotal)}</b></div><div className="flex justify-between"><span>Shipping</span><b className="text-white">{details.shipping === 0 ? "Free" : formatMoney(details.shipping)}</b></div><div className="flex justify-between"><span>Estimated tax</span><b className="text-white">{formatMoney(details.tax)}</b></div><div className="flex justify-between border-t border-white/15 pt-4 text-xl text-white"><span>Total</span><b>{formatMoney(details.total)}</b></div></div><button disabled={!details.count} className="mt-6 w-full rounded-full bg-[#0b5f59] px-7 py-4 font-semibold text-white disabled:bg-[#d7e3e1] disabled:text-[#334155]">Pay {formatMoney(details.total)}</button><p className="mt-4 text-xs leading-5 text-[#c7d2d0]">Facebook Pixel Purchase and CAPI-ready event data fires on mock completion with browser/server dedupe-friendly event IDs.</p></aside>
      </form>
